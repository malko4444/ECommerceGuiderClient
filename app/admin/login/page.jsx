...
import {
  FaLock, FaEnvelope, FaSignInAlt, FaUserPlus, FaStore,
  FaEye, FaEyeSlash, FaCheckCircle, FaExclamationCircle,
  FaShieldAlt, FaBolt, FaChartLine, FaRocket, FaUserShield
} from 'react-icons/fa';

const HIGHLIGHTS = [
  { icon: <FaBolt />,      title: 'Instant access',        desc: 'Jump straight into platform controls' },
  { icon: <FaChartLine />, title: 'Live platform stats',   desc: 'See sellers, orders & activity in real time' },
  { icon: <FaRocket />,    title: 'Secure session',        desc: 'HTTP-only cookies, auto-expiring JWTs' },
];
...
export default function AdminLogin() {
  ...
  const baseURL = `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/login`;
  ...
  const loginAdmin = async () => {
    ...
    try {
      ...
      if (remember) localStorage.setItem('rememberAdminEmail', email);
        ...
        <div className="relative z-10 p-10 xl:p-14 flex flex-col justify-between text-white w-full">
          ...
          <div>
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-3 py-1 mb-4 backdrop-blur">
              ...
              <span className="text-xs font-semibold text-amber-100">Restricted area</span>
            </div>
            <h1 className="text-4xl xl:text-5xl font-black leading-[1.1] tracking-tight">
              Welcome back, admin. <br />
              <span className="italic text-amber-200">Let&apos;s run the show.</span>
            </h1>
            <p className="text-teal-50 mt-5 text-base leading-relaxed max-w-md">
              Sign in to access platform controls, review activity, and manage
              sellers across ECommerce Guider.
            </p>
          ...
          <div className="flex items-center gap-3 bg-white/10 backdrop-blur rounded-xl p-3 border border-white/10 w-fit">
            ...
            <p className="text-xs">Unauthorized access attempts are logged.</p>
          </div>
        </div>
      </aside>
          ...
          <div className="bg-white rounded-2xl shadow-xl shadow-slate-900/5 border border-slate-100 p-6 sm:p-8">
            ...
            <div className="space-y-3.5">
              ...
              <p className="text-[11px] text-slate-500 text-center">
                Protected area. All sign-in attempts are recorded.
              </p>
            </div>
          </div>
